...
{
	//промежуточные значения не должны быть отрицательными см. evaluate
	//хоть данное правило и имеет определенные ограничения, в использовании оператора -+ есть логика только если он используется для 
